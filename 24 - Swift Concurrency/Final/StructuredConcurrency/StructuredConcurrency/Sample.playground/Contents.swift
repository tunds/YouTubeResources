...
struct ContentService {
    ...
    func fetchContent() async throws -> Content {
        let users = try await fetch(URL(string: "https://jsonplaceholder.typicode.com/users")!,
                                    type: [User].self)
        let posts = try await fetch(URL(string: "https://jsonplaceholder.typicode.com/posts")!,
                                    type: [Post].self)
        return (users, posts)
    }
